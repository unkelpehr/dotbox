...
const proto = {
	...
	unset (bitmask, path) {
		...
		const opts = dotbox.flags.parse(bitmask);
		const target = opts.WRITE ? this.written : this.changes; 
		...
		if (opts.WRITE) {
			...
			dotbox.set(bitmask | dotbox.flags.DELETE, target, path);
		} else {
